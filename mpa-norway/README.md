...
The tool is divided into 2 docker images, one for the `OpenDrift` tool and it's dependencies, and other for the `mpa-norway` itself.
...
## Run the mpa-norway tool

We can find the cwl repository for the `mpa-norway` tool in the following link: [mpa-norway](https://pipe-drive.inesctec.pt/application-packages/tools/mpa-norway).

To run directly with cwltool, you can use the following command:

```
cwltool https://pipe-drive.inesctec.pt/application-packages/tools/mpa-norway/mpa_norway_0_1_0.cwl#mpa-norway input.json
```

Can also be run with the with the local `tool.cwl` file:

```
cwltool tool.cwl#mpa-norway input.json
```
...
You can find the application description on [ILIAD registry](https://iliad-registry.inesctec.pt/collections/apks/items?name=mpa_norway). There you can also find the CWL reference on last fields `url`.
...
4) Update the Dockerfile for the `mpa-norway` image to use the new `Opendrift` image and install it's requirements.
